...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen flex bg-background">
        ...
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-card flex flex-col border-r border-border animate-in slide-in-from-left duration-200">
                ...
                <div key={gi}>
                  {group.label && (
                    ...
                    >
                      ...
                      <ChevronDown className={cn("h-3 w-3 text-muted-foreground/50 transition-transform", !openGroups[group.label] && "-rotate-90")} />
                    </button>
                  )}
            ...
            <div className="px-2 py-2 border-t border-border">
              <Button
                variant="ghost"
                className="w-full justify-start text-muted-foreground hover:text-foreground hover:bg-accent text-xs"
      ...
      <aside
        className={cn(
          "bg-card flex-col border-r border-border transition-all duration-200 hidden md:flex",
          collapsed ? "w-16" : "w-60"
        )}
      >
        {/* Logo */}
